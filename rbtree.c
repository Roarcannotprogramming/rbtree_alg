 ...
 * 比较两个节点是否重叠
 */
unsigned int overlap(Node *node1, Node *node2) {
    return (node1 -> key <= node2 -> high) && (node2 -> key <= node1 -> high);
}

Node* interval_search(RBTree tree, Node *node) {
    Node* x = tree;
    while(x != NULL && !overlap(x, node)) {
        if (x -> left && x -> left -> max >= node -> key) {
            x = x -> left;
        }
        else {
            x = x -> right;
...
Type get_max(Node *node) {
    if (node)
        return node->max;
    return 0;
}

Type max_value(Type a,Type b, Type c) {
    if (a > b) {
        if (a > c)
            return a;
        return c;
...
static void rbtree_left_rotate(RBRoot *root, Node *x)
{
    ...
    x->parent = y;
    // 修正max
    y->max = max_value(y->high, get_max(y->left), get_max(y->right));
    x->max = max_value(x->high, get_max(x->left), get_max(x->right));
 ...
 *     right 是右孩子。
 */
static Node* create_rbtree_node(Type key, Type high, Node *parent, Node *left, Node* right)
{
    Node* p;
 ...
 *     -1，插入失败
 */
int insert_rbtree(RBRoot *root, Type key, Type high)
{
    Node *node;    // 新建结点
    ...
    // 如果新建结点失败，则返回。
    if ((node=create_rbtree_node(key, high, NULL, NULL, NULL)) == NULL)
        return -1;
